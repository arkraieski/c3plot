...
The c3plot package implements various plotting functions with similar
...
the C3.js charting library. This makes it easy to generate interactive
versions of common plots without the cognitive overhead of
using/learning a plotting package with completely different arguments
and syntax. This vignette will help you get started creating interactive
visualizations with c3plot.
...
The [`c3plot()`](../reference/c3plot.md) function works much like base
...
![](c3plot-getting-started_files/figure-html/unnamed-chunk-3-1.png)

Now with [`c3plot()`](../reference/c3plot.md):
...
function, [`c3plot()`](../reference/c3plot.md) can also create line
...
For [`c3plot()`](../reference/c3plot.md) colors for the points can be
...
1.  Pass a factor the same length as `x` to the `col.group` argument and
    ...
    each group will be assigned a color by C3.js using its default
    palette.

2.  Pass a factor to `col.group` and vector of colors to use for each
    group to `col`.

First, let’s color a scatter plot by a factor but let C3.js pick the
colors:
...
c3plot also provides a [`c3pie()`](../reference/c3pie.md) function that
works like the base R `pie` function, albeit with a simplified set of
arguments. `x` is a vector of non-negative numerical quantities. Names
for the pie slices can be given as a character vector to the `labels`
argument. If `x` is a named vector, the names will be used to label the
pie slices (`labels` defaults to `names(x)`).
...
[`c3plot()`](../reference/c3plot.md) is a generic function with S3
methods for various classes of base R objects. While
[`c3plot()`](../reference/c3plot.md) doesn’t have as many methods, this
...
[`?c3plot.density`](../reference/c3plot.density.md) to view the help
file for [`c3plot.density()`](../reference/c3plot.density.md).

Let’s create a density plot with the Old Faithful geyser data using
[`c3plot.density()`](../reference/c3plot.density.md). First, create a
“density” object:
...
Since [`c3plot()`](../reference/c3plot.md) has a method for “density”
...
![](c3plot-getting-started_files/figure-html/unnamed-chunk-14-1.png)

The plots are very similar aside from some visual differences and
obviously the interactivity of the c3plot version.
[`c3plot()`](../reference/c3plot.md) methods are designed to replicate
the functionality of their Base R counterparts.

[`c3plot.function()`](../reference/c3plot.function.md) lets us do cool
stuff like this:
...
This package also provides a [`c3barplot()`](../reference/c3barplot.md)
...
bars to the argument `narmes.arg` (otherwise the x-axis will be numbered
...
[`c3plot.factor()`](../reference/c3plot.factor.md) method to make a bar
...
By now you should be more than ready to start using c3plot on your own
data. Consult the help pages for details about arguments and methods.
While c3plot doesn’t offer quite the same level of customization as base
